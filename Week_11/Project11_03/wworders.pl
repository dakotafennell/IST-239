...
# Retrieves the value of the skey parameter using a cgi object.
my $id = $cgi_object->param('id');
# Retrieves the value of the pwd parameter using a cgi object.
my $pwd  = $cgi_object->param('pwd');

# These are sample orders stored in JSON format. In a real application, these would be stored in a database.
my @customerOrders = (
    { id => "RW301",
      password => "kaboom",
      orders =>
...
# Loops through the customerOrders array
foreach my $row (@customerOrders) {
  # If the id and password match, print the orders
...
if ($count == 0) {
  # Prints, in JSON format, the status of the order if there are no orders found.
